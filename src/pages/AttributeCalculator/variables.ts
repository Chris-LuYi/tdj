...
export const calculateAttribute = (
  ...
  attrBasicSelectOptionsAry
    ...
    .forEach((attr) => {
      ...
      const attrWeapon = data.weapon?.attrBonus[attr] || 0;
      console.log(attrWeapon);
      const attrEquipment =
        data.equipped?.equipments
          ?.map((o) => {
            return o.attrBonus
      ...
      const attrSoulStone = data.soulStones
        .map((o) => {
          let v = 0;
      ...
      if (!soulStoneChanged && !panelFixedValueChange) {
        if (panelValueChanged) {
          const attrLevelAndStar =
            (data.attrFinal[attr] || 0) / totalPanelDataPct -
            attrWeapon -
            attrEquipment -
            attrWunei;
          ...
          newData.attrFinal[attr] =
            (data.attrRaw[attr] + attrWeapon + attrEquipment + attrWunei) * totalPanelDataPct;
      ...
      if (soulStoneChanged) {
        const attrSoulStonePct = data.soulStones
          .map((o) => {
            let v = 0;
